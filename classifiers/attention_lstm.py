...
class AttentionLSTM(nn.Module):
    def __init__(self, args, n_vocab, embed_dim, n_classes, dropout=0.5):
        ...
        self.v = nn.Parameter(torch.Tensor(attention_dim, 1))
    ...
    def attention(self, h, h_t, i_size, b_size):
        ...
        context = torch.bmm(h.t().transpose(1, 2), attention.t())
    ...
    def forward(self, inputs):
        ...
        out, h_t = self.lstm(embedded, state)  # out: [i, b, h]

        a, c = self.attention(out, out[-1], i_size, b_size)
